...
program define _grifvar, sortpreserve
	...
	* Syntax sextion. Indicates what can be used and what cannot
	...
	agini 													/// This are RIFS from HEckley Gerdthan and Kjellson Only one that is rank independent
	...
	** This checks how many options have been selected	Only 1 option is allowed
	local d=("`gini'"!="")+("`ginismall'"!="")+("`mean'"!="")+("`var'"!="")+("`std'"!="")+("`cvar'"!="")+("`q'"!="")+("`q2'"!="")+("`iqr'"!="")+("`iqr2'"!="")+("`iqratio'"!="")+ ///
			...
			("`agini'"!="")+("`acindex'"!="")+("`cindex'"!="")+("`eindex'"!="")+("`logvar'"!="")+ ///
	...
	*tip(p) pov()
	if "`tip'"!="" {
		...
		* TIP ordinate
		by `touse' `by':gen double `itip' =sum(((100*`pvar'[_N])<=(`tip'/100))*(`pline'>=`exp')*(`pline'-`exp')*`weight'/`tot_wt')
		* THE RIF
...
if "`rifown'"!="" {
	...
	if "`hvarp'"!="" {
		** half variance +
		sort `touse' `by' `exp' `sortseed'
		tempvar pvar pvarm cum_wt tot_wt glp hvp mns absmn mpvar
		by `touse' `by':gen double `cum_wt'=sum(`weight') 
		by `touse' `by':gen double `tot_wt'=`cum_wt'[_N]
        by `touse' `by':gen double `glp'=sum(`exp'*`weight'/`tot_wt')
		by `touse' `by':gen double `mns'=`glp'[_N]
		by `touse' `by':gen double `absmn'=sum(abs(`exp'-`mns')*2*`weight'/`tot_wt')
		by `touse' `by':gen double `pvar' = `cum_wt'/`tot_wt'
		by `touse' `by':gen double `pvarm' = sum(`weight'/`tot_wt'*(`exp'<=`mns')) 
		by `touse' `by':gen double `mpvar' = sum(`weight'/`tot_wt'*`pvarm'*(1-`pvarm')) 
		*noisily sum `exp' `mns' `pvarm'
		*gen iz1=-2*(ix1-$mn)*(1-p)+i2*(ix1-$mn)^2
		by `touse' `by': gen `typlist' `varlist' = -2*`absmn'*(`exp'-`mns')*`mpvar'+(`exp'>=`mns')*(`exp'-`mns')^2   if `touse'
		label var `varlist' "RIF for Positive Half variance of `exp'"
	}
	*replace ifxx=(((ix1>6.913379)+.575-1)*(ix1-6.913379))^2+(ix1>6.913379)*(ix1-6.913379)
	*replace ifxy=((-(ix1<6.913379)-.575+1)*(ix1-6.913379))^2-2*(ix1<6.913379)*(ix1-6.913379)
	if "`hvarn'"!="" {
		** half variance +
		sort `touse' `by' `exp' `sortseed'
		tempvar pvar pvarm cum_wt tot_wt glp hvp mns
		by `touse' `by':gen double `cum_wt'=sum(`weight') 
		by `touse' `by':gen double `tot_wt'=`cum_wt'[_N]
        by `touse' `by':gen double `glp'=sum(`exp'*`weight'/`tot_wt')
		by `touse' `by':gen double `mns'=`glp'[_N]
		by `touse' `by':gen double `pvar' = `cum_wt'/`tot_wt'
		by `touse' `by':gen double `pvarm' = sum(`weight'/`tot_wt'*(`exp'<=`mns')) 
		*gen iz2=2*(ix1-$mn)*(p)+i1*(ix1-$mn)^2
		by `touse' `by': gen `typlist' `varlist' = 2*(`exp'-`mns')*(`pvarm'[_N])+(`exp'<=`mns')*(`exp'-`mns')^2   if `touse'

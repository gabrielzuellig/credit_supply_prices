...
#  Output:
#  
#   - temp/cdf_prodcom.dta
#     sigmaBW cf_EU cdf_DK cdf_DK_food cdf_DK_cpg, needed for Fig. 4c
#
#   - Tab C.1
...
rm(conv2, conv1)
# prodcom now includes all the original prodcodes translated to HS 2010, translated to HS 2007, translate to SITC Rev. 3, 
# at which level we observe demand elasticities
...
write.csv(cdf_all, 'temp/cdf_prodcom.csv', row.names=F)
write.dta(cdf_all, 'temp/cdf_prodcom.dta')

rm(cdf_all)

############################################################################################
## sigmaBW of consumer packaged goods
############################################################################################
...
write.table(cpg, file = 'out/TabC1_cpg_definition.txt', sep = ',', row.names=F)
